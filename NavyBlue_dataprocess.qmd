...
custom_thousand_format <- Vectorize(function(x, decimalPoint = 0.001) {
  if (abs(x) >= 1e6) {
    scales::number_format(accuracy = decimalPoint, big.mark = ",", suffix = "m")(x / 1e6)
  } else {
    scales::number_format(accuracy = decimalPoint, big.mark = ",", suffix = "k")(x / 1e3)
  }
})
...
mot <- mot |>
  mutate(
    `2000` = custom_thousand_format(`2000`, decimalPoint = 0.01),
...
gt_table <- mot |>
  ...
  gtExtras::gt_plt_sparkline(
    ...
    fig_dim = c(5, 20),
    type = "shaded",
    palette = c("black", rep("transparent",3), "grey"),
    label = FALSE
  ) |>
  ...
  cols_label(`Sparkline` = "Trend") |> # Rename sparkline to trend
  ...
  text_transform(
    ...
    fn = function(x) {
      ...
      ifelse(as.numeric(x) > 0,
        ...
        paste0('<span style="color:red;">&#9660;</span> ', abs_x, "%")
      ) # HTML for downward arrow
    }
  )
...
```{r}
#| label: Add Column Spanner Labels

gt_table <-
  tab_spanner(
    gt_table,
    label = "Change and Trends (2000 - 2020)",
    id = "changer",
    columns = c(`%`, `Sparkline`)
  ) |>
  tab_style(
...
```{r}
#| label: Change column header style
...
```{r}
#| label: set public, private, other total with style
...
gt_table <- gt_table |> 
  tab_style( 
    ...
    locations = cells_body(
      rows = c(4, 10, 13) # Assuming the selected rows are summary rows
    )
  ) |>
  tab_options(quarto.disable_processing = TRUE)
...
```{r}
#| label: add border right on the stub
...
```{r}
#| label: Add Footnote to the table

gt_table <- gt_table |>
  tab_footnote(
    footnote = html("Removed Rows:<br>1. One Mode Of Transport,<br>2. Two Or More Modes Of Transport,<br>3. MRT & Car Only,<br>4. MRT & Another Mode,<br>5. Other Combinations Of MRT Or Public Bus,<br>6. Other combinations of MRT/LRT or Public Bus"),
    locations = cells_column_labels("Categories") # Assuming the footnote is for the Categories column
  ) |>
  tab_style(
    list(
      cell_text(
...
gt_table <- gt_table |>
  tab_source_note(
    source_note = "Source: Department of Statistics, Singapore" # add the source note for the entire table
  ) |>
  tab_style(
    list(
      cell_text(
...
gtsave(gt_table, file = "output_gt_table.png")
```
